...
void printorder(struct binary* node){
     ...
     printf("String is: %s frequency is: %d \n", node->key->keyS, node->key->frequency );
}
...
void searcher(char *stringWord, struct binary *n){
...
//sees what string compared to then goes to right subtree 
        ...
        printf("String is compared with %s (%d), go to the right subtree\n", n->key->keyS, n->key->frequency);
     
        searcher(stringWord, n->right);
      }
...
//if not found and not right subtree is goes to left
          ...
          printf("String is compared with %s (%d), go to the left subtree\n", n->key->keyS, n->key->frequency);
        searcher(stringWord, n->left);
      }
}
